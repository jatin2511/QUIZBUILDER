...
function QuizPage() {
  ...
  const fetchQuizAnalytics = async () => {
    try {
      const response = await axios.get(`http://quizbuilder-env.eba-qrrtm5dv.ap-south-1.elasticbeanstalk.com/api/quiz/takequiz/${quizid}`, {
        headers: {
  ...
  const updateQuizImpression = async () => {
    try {
      await axios.post(`http://quizbuilder-env.eba-qrrtm5dv.ap-south-1.elasticbeanstalk.com/api/quiz/impression/${quizid}`);
    } catch (error) {
  ...
  const updateQuestionAttempt = async (questionId) => {
    try {
      if (questionId && !hasUpdatedAttempt.current.has(questionId)) {
        await axios.post(`http://quizbuilder-env.eba-qrrtm5dv.ap-south-1.elasticbeanstalk.com/api/quiz/${quizid}/question/impression/${questionId}`);
  ...
  const updateCorrectAnswerCount = async (questionId) => {
    try {
      if (questionId) {
        await axios.post(`http://quizbuilder-env.eba-qrrtm5dv.ap-south-1.elasticbeanstalk.com/api/quiz/${quizid}/question/impression/correctanswer/${questionId}`);
      }
    } catch (error) {
  ...
  const updateOptionImpression = async (questionid, optionId) => {
    try {
      if (questionid) {
        await axios.post(`http://quizbuilder-env.eba-qrrtm5dv.ap-south-1.elasticbeanstalk.com/api/quiz/optionincreament/${quizid}/${questionid}/${optionId}`);
      }
    } catch (error) {
      console.error('Error updating correct answer count:', error);
    }
